...
public class AccessTokenEncoderDecoderImpl implements AccessTokenEncoder, AccessTokenDecoder {
    ...
    public String encode(AccessTokenDTO accessTokenDTO) {
        ...
        return Jwts.builder().setSubject(accessTokenDTO.getSubject()).setIssuedAt(Date.from(now)).setExpiration(Date.from(now.plus(30, ChronoUnit.MINUTES))).addClaims(claimsMap).signWith(key).compact();
    }
    ...
    public AccessTokenDTO decode(String accessTokenEncoded) {
        try {
            ...
            return AccessTokenDTO.builder().subject(claims.getSubject()).roles(roles).userId(claims.get("userId", Long.class)).build();
